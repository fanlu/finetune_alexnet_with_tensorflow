...
import numpy as np
import glob
from datagenerator import input_pipeline, dense_to_one_hot, slice_input_pipeline
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        filenames = "/Users/lonica/Downloads/train.txt"
        # print(len(filenames))
        examples, labels = slice_input_pipeline(filenames, batch_size, num_epochs=num_epochs)
        with tf.Session() as sess:
            ...
            with tf.name_scope("accuracy"):
                correct_pred = tf.equal(tf.argmax(score, 1), tf.argmax(labels, 1))
